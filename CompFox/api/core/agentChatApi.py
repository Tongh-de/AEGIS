...
from CompFox.models.pojo.agentChatBo import AgentChatParams
...
router = APIRouter(prefix="/compfox/agent")
...
async def agent_chat(params: AgentChatParams):
    ...
    - chat: 合规咨询
    - recommend_questions: 推荐考题
    - learning_progress: 培训进度查询
    ...
    return HttpResponse.ok({
        'intents': [
            ...
            },
            {
                'type': 'chat',
                'name': '合规咨询',
                'description': '合规相关的普通对话',
                'params': ['topic']
            },
            {
                'type': 'recommend_questions',
                'name': '推荐考题',
                'description': '根据培训情况推荐练习题',
                'params': ['subject', 'weak_points']
            },
            {
                'type': 'learning_progress',
                'name': '培训进度',
                'description': '查询培训进度和统计',
...
def register_agent_chat_router(app):
    """注册 Agent 聊天路由"""
    app.include_router(router, tags=["CompFox - 合规助手"])
