...
from CompFox.models.pojo.questionBo import AiJudgeQuestionBo, QuestionRandomBo, QuestionImportBo
from CompFox.models.pojo.questionPo import QuestionPo
from CompFox.models.pojo.questionVo import QuestionRandomParamVo
from CompFox.services.questionService import get_question_service

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/compfox/question")
...
def get_subjects():
    ...
    try:
        ...
        logger.error(f"获取合规领域列表失败：{str(e)}")
        return HttpResponse.error(f"获取合规领域列表失败：{str(e)}")


@router.get("/random_one")
def get_random_one_question(
    user_id: str = Query(None, description="员工ID"),
    ...
    grade: Optional[int] = Query(None, description="职级 1-4"),
...
async def generate_random_question(params: QuestionRandomParamVo):
    """
    随机AI生成题目接口
    :param params:  如果不指定对应参数的值，对应参数将随机取值。 如 不指定 合规领域，将随机取一个合规领域
    :return:
    """
...
def search_questions(
    ...
    subject: Optional[str] = Query(None, description="合规领域"),
    question_type: Optional[str] = Query(None, description="题型"),
    grade: Optional[int] = Query(None, description="职级"),
...
def create_question(
    ...
    grade: int = Form(..., description="职级"),
    subject: str = Form(..., description="合规领域"),
...
async def import_questions_from_text(params: QuestionImportBo):
    ...
    用户输入包含考题的文本，AI 自动解析并批量插入到题目表。

    Args:
        params: 导入参数
            - text: 包含考题的文本
            - subject: 合规领域（必填）
            - grade_range: 职级范围（如 "1-2" 初级，"2-3" 中级，"3-4" 高级/管理层）
            - difficulty: 默认难度（1-5）
...
async def import_questions_from_file(
    ...
    subject: str = Form(..., description="合规领域"),
    grade_range: str = Form("1-2", description="职级范围"),
