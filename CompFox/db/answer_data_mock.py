...
def _generate_single_record(_) -> Dict:
    ...
    if question:
        # 使用真实题目生成答题记录
        prompt = f"""请根据以下真实合规考题，生成一条员工答题记录数据。
...
- 合规领域: {question.subject}
- 职级: {question.grade}
- 难度等级: {question.difficulty_level}
- 标准答案: {question.answer}
- 合规知识点: {question.knowledge_points}

请生成：
1. 员工答案（符合题型和题目内容，不一定都要正确）
2. 得分率 (0-1，要有变化)
3. AI 评卷置信度 (0.7-0.99)
4. AI 评卷简要评语

请以 JSON 格式返回：
{{"question_id": {question.id}, "question_uuid": "{question.question_uuid}", "question_type": "{question.question_type}", "user_answer": "员工答案", "score": 0.8, "ai_confidence": 0.95, "ai_comment": "评卷评语"}}
"""
    else:
        # 无真实题目时的降级方案
        question_types = ["single_choice", "multiple_choice", "fill_blank", "short_answer"]
        prompt = """请生成 1 条员工合规考核答题记录数据，包含：
