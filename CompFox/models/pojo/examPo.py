...
class ExamPo(DefaultDbModel):
    ...
    考试记录表实体类
    """
    table_alias: ClassVar[str] = 'compfox_exams'
    create_table_sql = f"""
        -- 合规考核记录表
        CREATE TABLE IF NOT EXISTS `{table_alias}` (
            `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '考核记录 ID',
            `exam_uuid` VARCHAR(36) NOT NULL DEFAULT (UUID()) COMMENT '考核 UUID',

            -- 关联信息
            `paper_id` BIGINT UNSIGNED NOT NULL COMMENT '试卷 ID',
            `user_id` VARCHAR(100) NOT NULL COMMENT '员工姓名/工号',
            ...
            -- 答案信息
            `answers` JSON COMMENT '员工答案（JSON 格式，key 为题目 ID，value 为员工答案）',
            ...
            -- 合格判定（新增）
            `passed` TINYINT DEFAULT NULL COMMENT '是否合格：0=不合格 1=合格',

            -- AI 评判
            `ai_summary` TEXT COMMENT 'AI 考核总结（含复训建议）',
            `ai_scoring_basis` TEXT COMMENT 'AI 打分依据',
            `teacher_review` TEXT COMMENT '评审专家复核意见（保留字段）',
        ...
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='合规考核记录表';
        """
    ...
    user_id: str = Field(..., description="员工姓名/工号")
    ...
    # 合格判定
    passed: Optional[int] = Field(None, description="是否合格：0=不合格 1=合格")

    # AI 评判
    ai_summary: Optional[str] = Field(None, description="AI 考核总结")
