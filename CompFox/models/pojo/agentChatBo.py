    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "reason": "用户明确要求出题，并指定了合规领域、职级和难度"
            }
        }
...
class RecommendQuestionsParams(BaseModel):
    ...
    """
    subject: Optional[str] = Field(None, description="合规领域")
    weak_points: Optional[str] = Field(None, description="薄弱合规知识点")
...
class ImportQuestionsParams(BaseModel):
    ...
    grade_range: str = Field("1-2", description="职级范围")
