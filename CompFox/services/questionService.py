...
from CompFox.prompts.questionPrompts import ai_judge_prompt, SM_QUESTION_GENERATE_PROMPT, IMPORT_QUESTION_PROMPT
...
class QuestionService(BaseModel):
    subjects: List[str] = Field([], description="合规领域列表")
    ...
    def get_subjects(self):
        if not self.subjects:
            self.subjects = BaseParamsModel.get_params_by_parent_code('compfox_subject', 'CompFox') or []
        return self.subjects

    def get_question_types(self):
        if not self.question_types:
            self.question_types = BaseParamsModel.get_params_by_parent_code('compfox_question_type', 'CompFox') or []
        return self.question_types

    def get_difficulty_levels(self):
        if not self.difficulty_levels:
            self.difficulty_levels = BaseParamsModel.get_params_by_parent_code('compfox_difficulty_label',
                                                                               'CompFox') or []
    ...
    def get_knowledge_points(self, grade: str, subject: str):
        ...
        if not self.knowledge_points.get(code):
            # 从数据库查询知识点
            try:
                knowledge_point = BaseParamsModel.get_param_by_code(code=code, type='CompFox',
                ...
                if knowledge_point:
                    ...
                    logger.warning(f"未找到知识点: code={code}, type=CompFox, parent_code=compfox_knowledge_point")
    ...
    def get_question_rule(question_type: str):
        ...
        result = ''
        try:
            result = BaseParamsModel.get_param_by_code(question_type, 'CompFox')
        except Exception as e:
    ...
    def random_generate_question(self, question_random_bo: QuestionRandomBo):
        ...
        grade_type = question_random_bo.grade_type or random.choice(['初级', '中级', '高级'])
        ...
        try:
            ...
            search_query = f"{subject} {grade_type} {' '.join(knowledge_points[:3])}"
            regulation_context = ks.retrieve_context(search_query, limit=3)
            if regulation_context:
                logger.info(f"RAG 检索成功，法规上下文长度：{len(regulation_context)}")
        except Exception as e:
            logger.warning(f"RAG 检索失败，降级为无 RAG 模式：{e}")

        user_prompt = f"""帮我出一道合规考题，根据提供的知识点任选一个或多个进行出题：
        合规领域：{subject}
        题型：{question_type}
        难度：{difficulty_level}
        职级：{grade_type}
        ...
        # 规范化 LLM 输出：将中文名/前缀编码转换为标准代码
        response = self._normalize_llm_output(response, subject, question_type, grade_type)

        question = QuestionPo(ai_model=llm.model_name, ai_prompt=str(messages), created_by=505, **response)
        question.save()
        return response

    def _normalize_llm_output(self, data: dict, target_subject: str, target_type: str, target_grade: str) -> dict:
        """规范化 LLM 输出，确保字段值符合数据库约束"""
        # subject → 标准代码 (如 aml, data_security)
        subject_map = {s['desc']: s['value'] for s in self.get_subjects()}
        subject_map.update({s['code']: s['value'] for s in self.get_subjects()})
        raw_subject = str(data.get('subject', target_subject))
        data['subject'] = subject_map.get(raw_subject, target_subject)[:30]

        # question_type → 标准代码 (如 single_choice, multiple_choice)
        type_map = {t['desc']: t['value'] for t in self.get_question_types()}
        type_map.update({t['code']: t['value'] for t in self.get_question_types()})
        raw_type = str(data.get('question_type', target_type))
        data['question_type'] = type_map.get(raw_type, target_type)[:30]

        # grade → 整数 1-4
        grade_map = {'初级': 1, '中级': 2, '高级': 3, '管理层': 4}
        raw_grade = data.get('grade', target_grade)
        if isinstance(raw_grade, str):
            data['grade'] = grade_map.get(raw_grade, 1)
        elif isinstance(raw_grade, (int, float)):
            data['grade'] = int(raw_grade)

        # difficulty_label → 标准值
        diff_label_map = {'basic': 'basic', 'intermediate': 'intermediate', 'advanced': 'advanced',
                          'easy': 'basic', 'medium': 'intermediate', 'hard': 'advanced',
                          '基础': 'basic', '中级': 'intermediate', '高级': 'advanced'}
        raw_diff = str(data.get('difficulty_label', 'basic'))
        data['difficulty_label'] = diff_label_map.get(raw_diff, 'basic')[:10]

        # difficulty_level → 1-5
        try:
            data['difficulty_level'] = min(5, max(1, int(data.get('difficulty_level', 3))))
        except (ValueError, TypeError):
            data['difficulty_level'] = 3

        # 截断字符串字段到字段最大长度
        for field, max_len in [('question_text', 65535), ('answer', 65535), ('analysis', 65535),
                                ('hint', 500), ('knowledge_points', 500), ('ai_judge_prompt', 65535)]:
    ...
    def ai_judge_question(params: AiJudgeQuestionBo):
        ...
        try:
            ...
            logger.warning(f"RAG 判题检索失败，降级：{e}")

        system_prompt = prompt_render(ai_judge_prompt, {**question.model_dump(), "regulation_context": regulation_context})
    ...
    def ai_judge_question_with_data(question_data: dict, user_answer: str, user_id: str, source: str = 'agent_chat'):
        ...
        try:
            ks = get_knowledge_service()
            search_query = f"{question_data.get('subject', '')} {question_data.get('question_text', '')}"[:500]
            regulation_context = ks.retrieve_context(search_query, limit=3)
        except Exception as e:
            logger.warning(f"RAG 判题检索失败，降级：{e}")

        system_prompt = prompt_render(ai_judge_prompt, {**question_data, "regulation_context": regulation_context})
