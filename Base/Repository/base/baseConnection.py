...
class BaseConnection(ABC):
    ...
    def execute(
        ...
        try:
            ...
            if ie.args[0] == 1062:
                logger.warning(f"唯一键冲突（数据已存在）：{ie}")
            else:
                logger.warning(f"数据完整性错误：{ie}")
            if operation_type == OperationType.QUERY:
                return []
            return -1
        except OperationalError as oe:
            err_code = oe.args[0] if oe.args else 0
            if err_code == 1050:
                logger.debug(f"表已存在，跳过创建表：{oe}")
                return 0
            # Non-connection errors (data-level): don't poison connection
            if err_code < 2000:
                logger.warning(f"SQL 执行失败：{oe}")
                if operation_type == OperationType.QUERY:
                    return []
                return -1
            # Connection-level errors: poison connection
            logger.warning(f"SQL 执行失败，标记连接为不可用：{oe}")
            self._is_available = False
            if operation_type == OperationType.QUERY:
                return []
            return -1
            ...
            # Don't poison connection for unexpected errors — could be data issues
            if operation_type == OperationType.QUERY:
                return []
            elif operation_type == OperationType.INSERT:
                return -1
